...
[ -n "${CORE_PLUGIN_ROOT:-}" ] && [ -f "$CORE_PLUGIN_ROOT/hooks/lib/gate-lib.sh" ] || {
  echo "run-finding-gate-tests: cannot find core canon gate-lib.sh; set CORE_PLUGIN_ROOT to a checkout of tokenmaxxxer-core (core issue #72) before running this suite." >&2
  exit 1
}
